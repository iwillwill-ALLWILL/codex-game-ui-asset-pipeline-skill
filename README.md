...
- 从一张参考图扩展同风格 UI
- 从 UI 截图或概念图里试拆组件
- 按项目风格库做简单 UI 帧动画，例如按钮呼吸、图标发光、奖励闪烁、进度条扫光、加载转圈、面板弹出
- 清理素材边缘粉色/绿色背景颗粒
...
| 动态化 UI 组件 | 想让按钮、图标、奖励、进度条有简单动效 | `使用 $game-ui-asset-pipeline，读取我的项目风格库，把这个按钮做成 8 帧循环呼吸发光动画。动效颜色、光效、粒子和材质要和原 UI 一致，输出透明 PNG 帧、preview.gif 和目标引擎接入说明。` |
...
| UI 动画统一 | 每帧同尺寸、同锚点、边缘干净，有 `preview.gif`，循环首尾不跳；光效、扫光、粒子、材质必须和项目风格库一致 |
...
```text
使用 $game-ui-asset-pipeline，读取 <风格名> 风格库，把这个 UI 组件做成简单帧动画。

组件：<比如：开始按钮 / 金币图标 / 奖励卡框 / 进度条填充>
动效：<比如：8 帧循环呼吸发光 / 扫光 / 闪烁 / 加载转圈 / 面板弹出>
目标引擎：<自动识别项目 / Godot / Unity / Cocos / generic>

要求：
1. 先读取项目风格库和原始静态组件，不要另起一套特效风格。
2. 动效的颜色、发光、扫光形状、粒子密度、材质纹理、描边粗细都要和现有 UI 一致。
3. 每帧同尺寸、同锚点、同透明边距，基础组件不要变形跑样。
4. 默认不要把文字烘焙进帧里，文字交给引擎文本节点。
5. 输出透明 PNG 帧、静态 fallback、preview.gif 或 contact_sheet，以及目标引擎接入说明。
```
